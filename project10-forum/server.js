...
app.get('/', async (req, res) => {
    try {
        ...
        console.error(err);
        res.send('포스트 목록을 불러오는 중 오류가 발생했습니다.');
    }
});
...
app.post('/posts/new', async (req, res) => {
    ...
    try {
        ...
        console.error(err);
        res.render('new', { error: '포스트 작성 중 오류가 발생했습니다.' });
    }
});
...
app.get('/posts/:id', async (req, res) => {
    ...
    try {
        ...
        if (!post) {
        return res.send('포스트를 찾을 수 없습니다.');
        }
        res.render('detail', { post });
    } catch (err) {
        console.error(err);
        res.send('포스트 상세 정보를 불러오는 중 오류가 발생했습니다.');
    }
});

// 포스트 수정 페이지 (작성자만 접근 가능)
app.get('/posts/:id/edit', async (req, res) => {
    ...
    try {
        ...
        console.error(err);
        res.send('포스트를 불러오는 중 오류가 발생했습니다.');
    }
});
...
app.post('/posts/:id/edit', async (req, res) => {
    ...
    if (!title || !content) {
        const post = await Post.findById(postId);
        return res.render('edit', { post, error: '모든 필드를 입력해주세요.' });
    }
